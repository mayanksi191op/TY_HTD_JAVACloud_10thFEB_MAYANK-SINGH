	...
	public static void loginController() {
		...
		while (!exit) {
			...
			case "1":
				...
				try {
					if (!validationClass.usernameValid(custUsername)) {
			...
			case "2":
				...
				while(!count) {
					...
					try {
						if (!validationClass.usernameValid(empUsername)) {
							throw new InvalidUsernameException("enter valid username (letters and numbers, max-15)");
						} else {
							count = true;
						}
			...
			case "3":
				...
				while (!flag) {
					...
					try {
						if (!validationClass.nameValid(firstname)) {
				...
				while (!flag) {
					...
					try {
						if (!validationClass.nameValid(lastname)) {
				...
				while (!flag) {
					...
					try {
						if (!validationClass.usernameValid(username)) {
							throw new InvalidUsernameException("Please follow the pattern (can have only numbers or letters");
						} else {
							try {
								if (!FactoryClass.getLoginServices().usernameExists(username)) {
				...
				while (!flag) {
					...
					try {
						if (!validationClass.mailValid(email)) {
							throw new InvalidEmailException("Please enter a valid email. Ex :- mark@gmail.com");
						} else {
							try {
								if (!FactoryClass.getLoginServices().emailExists(email)) {
				...
				while (!flag) {
					...
					try {
						if (!validationClass.passValid(password)) {
							throw new InvalidPasswordException("Please enter minimum eight characters, at least one uppercase letter, one lowercase letter, one number and one special character");
						} else {
							flag = true;
						}
					} catch (Exception e) {
				...
				while (!flag) {
					...
					try {
						if (!validationClass.phoneValid(phoneString)) {
							throw new InvalidPhoneException("Invalid phone number(10 digits)!!! Enter valid phone number starting from 6.");
						} else {
				...
				while (!flag) {
					...
					try {
						if (!validationClass.fullNameValid(occupation)) {
				...
				while(!flag) {
					...
					if(!validationClass.doubleValid(stringBalance)){
				...
				if(FactoryClass.getLoginServices().register(occupation, dob, gender, username, email, password, firstname,
