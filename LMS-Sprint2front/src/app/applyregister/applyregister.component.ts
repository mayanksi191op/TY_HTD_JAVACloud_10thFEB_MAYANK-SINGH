...
export class ApplyregisterComponent implements OnInit {
  ...
  getLoans(){
    this.adminService.getLoan().subscribe(response =>{
  ...
  postCustomer(form: NgForm){
       ...
       // apply.applyloans["form.value.loantype", form.value.state, form.value.city, form.value.pincode, form.value.card, form.value.occupation, form.value.monthlyincome, form.value.loanamount, form.value.loantenure, form.value.address, form.value.status, form.value.mobile ];
